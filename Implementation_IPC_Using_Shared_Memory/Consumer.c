...
int main()
{
    ...
    shmid = shmget(6777, sizeof(int) * (SIZE + 3), 0666);
    ...
    while (1)
    {
        ...
        // Watchdog: Check if the producer process is still alive
        // kill(pid, 0) checks for process existence without actually killing it
        ...
        while (buf[10] == buf[11])
        {
            ...
            // Re-check inside the inner wait loop so consumer doesn't get stuck if empty
            if (kill(prod_pid, 0) == -1 && errno == ESRCH)
            {
